"""
Shared utilities for the generic AI vision pipeline.
...
# â”€â”€ IMAGE FORMAT NORMALISATION â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

# Common formats accepted by most remote vision/render backends.
_API_SUPPORTED: frozenset[str] = frozenset({"image/jpeg", "image/png", "image/webp", "image/gif"})

# PIL format name â†’ MIME type (for supported formats only)
...
def _normalise_image(data: bytes) -> tuple[str, bytes]:
    """
    Detect the true image format and ensure it is accepted by remote AI backends.

    - Supported formats (JPEG, PNG, WEBP, GIF) â†’ returned as-is with correct MIME.
    - Unsupported formats (HEIC, BMP, TIFF, AVIF, â€¦) â†’ converted to PNG via PIL.
    - Corrupt / unreadable data â†’ raises ValueError with a user-friendly message.

    Returns (mime_type, image_bytes).
    """
    from PIL import Image, UnidentifiedImageError  # local import â€” optional dep
    ...
    try:
        ...
        img.load()  # fully decode â€” raises on truncated / corrupt files
    ...
    if real_mime:
        # PIL confirmed a supported format â€” trust it over magic bytes if they differ
        return real_mime, data

    # Unsupported format (HEIC, HEIF, BMP, TIFF, AVIF, PCX, â€¦) â†’ convert to PNG
...
# â”€â”€ LOCAL UPLOAD â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ SUPABASE UPLOAD â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def upload_to_supabase(data_url: str) -> Optional[str]:
    ...
    if not supabase_url or not supabase_key:
        logger.info("Supabase upload skipped: SUPABASE_URL or SUPABASE_KEY is not configured.")
        return None
    ...
    try:
        ...
        logger.warning(
            "Supabase upload failed for bucket %r: %s: %s",
            bucket,
            type(exc).__name__,
            exc,
        )
